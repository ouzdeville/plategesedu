...
foreach($resultat as $row) {
?>
 <a href="saisienote.php?eval=<?php echo $row["TYPE"]."-".$row["ID_EVE"] ?>&ideve=<?php echo $row["ID_EVE"] ?>&NOM=<?php	echo $_GET["NOM"]  ?>&semestre=<?php	echo $semestre  ?>&classe=<?php	echo $classe  ?>&idmatierepromo=<?php echo $_GET["idmatierepromo"] ?>" class="btn btn-primary btn-lg active" role="button" aria-pressed="true"><?php	echo $row["TYPE"]."/".$row["ID_EVE"]  ?></a>
 <?php	} ?>
  </section>
 <section class="g">
 <ul>
